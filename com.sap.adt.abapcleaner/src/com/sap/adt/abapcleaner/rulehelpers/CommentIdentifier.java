...
/** 
 Determines whether an asterisk * comment contains (English / German) text or ABAP code. 
...
public class CommentIdentifier {
   ...
   private static char[] stringEndChars = new char[] {ABAP.QUOT_MARK};
   private static char[] stringEndChars2 = new char[] {ABAP.QUOT_MARK2};
   private static char[] stringTemplateEndChars = new char[] {ABAP.BRACE_OPEN, ABAP.PIPE};

	private final static String[] wordFreqRemoveTexts = new String[] { 	
	...
	private final static String[] wordFreqSkipTexts = new String[] { 
   ...
   private static HashSet<String> textSeparators;
   // private static HashSet<String> ambiguousSeparators;

   private static String[] autoGeneratedPrefixes;

   private static HashSet<String> levelOpeners;
   private static HashSet<String> levelClosers;
   ...
   private HashSet<String> lineSamples = new HashSet<>();
	...
	private static class Line {
		...
		Line(int indent, String text, String comment, String[] words, BitSet wordsMayBeKeywords, String[] separators, boolean isQuotMarkCommentLine) {
			this.indent = indent;
			// this.text = text;
			...
			this.isQuotMarkCommentLine = isQuotMarkCommentLine;
		}

		final int expectedIndentChangeForNextLine() {
			if (words == null || words.length == 0)
				return 0;

			if (levelOpeners.contains(getHashSetKey(words[0])))
				return AbapCult.stringEquals(words[0], "TRY", true) ? 4 : 2;
			else
				return 0;
		}

		final int expectedIndentChangeForThisLine() {
			if (words == null || words.length == 0)
				return 0;
			if (levelClosers.contains(getHashSetKey(words[0])))
	...
	private static void initialize() {
	   ...
	   textSeparators = initHashSet(new String[] {" (", ". ", "/", "; ", "B! <", "=E", "..", ";E", " @ ", " ..", "% ", " & ", "), ", ".", "B! @", "...", "B(", "% / ", " #", "%E", "): ", "B... ", "..., ", " _", "B! ", "?E", " ...E", " # ", "% + ", "% * ", ")", " +", "):E", " ...", "); ", "B# ", " ... ", ")):E", "%, ", "% - ", "% = ", " *", ": (", "B..", "* ", "))E", "B--- ", " (= ", "!E", "*", "). ", "B---", "B----- ", "!)E", ");E", "%) ", ". (", "B-", "B=> ", " --> ", "/ ", " -> -", ". - ", ") - (", "+ ", "+)E", "- ", " [", "] ", "!!!E", "? ", "%) * ", " [+ ", ".) ", "(!) ", ") & ", "#", "B!!! ", " !!!E", "+E", "/_", "% / +", ") -> ", " :", ".)E", "*E", ": ..", "; ..", "B_", "***E", "B-----", "!!!!!!!!E", " :E", "B@", "B------ ", ")!!!E", "..E", ")!E", ", ..", ") ... ", "... ", "B*", " %E", " (=", "->...~", ") => ", " -> ..", "B<< ", "B>> "});
	   // ambiguousSeparators = initHashSet(new string[] { ".E", "-", " = ", ") ", ",E", ": ", ").E", ") ", " - ", " / ", " + ", " * ", " < ", ", ", ")E", "B- ", "B* ", ", ", ":E", " -", "(", "),E", "B- ", "B* " });

	   autoGeneratedPrefixes = new String[] {"* <SIGNATURE>", "* | ", "* +-", "*&"};

	   levelOpeners = initHashSet(new String[] {"INTERFACE", "CLASS", "PUBLIC", "PROTECTED", "PRIVATE", "METHOD", "DO", "WHILE", "LOOP", "AT", "IF", "ELSEIF", "ELSE", "CASE", "WHEN", "CATCH", "FORM", "FUNCTION", "MODULE", "TRY", "EXEC", "TEST-SEAM", "TEST-INJECTION", "DEFINE", "ENHANCEMENT", "ENHANCEMENT-SECTION", "PROVIDE", "ON" });
	   levelClosers = initHashSet(new String[] {"ENDINTERFACE", "ENDCLASS", "PUBLIC", "PROTECTED", "PRIVATE", "ENDMETHOD", "ENDDO", "ENDWHILE", "ENDLOOP", "ENDAT", "ENDIF", "ELSEIF", "ELSE", "WHEN", "CATCH", "ENDFORM", "ENDFUNCTION", "ENDMODULE", "ENDTRY", "ENDCASE", "ENDEXEC", "END-TEST-SEAM", "END-TEST-INJECTION", "END-OF-DEFINITION", "ENDENHANCEMENT", "END-ENHANCEMENT-SECTION", "ENDPROVIDE", "ENDON" });
		isInitialized = true;
	}
   ...
   public static HashMap<String, String> getFrequentBritishEnglish() {
	...
	public final String filterForMode(String text, CommentIdentifierMode mode) {
		boolean allowAsterisk = (mode == CommentIdentifierMode.ASTERISK_COMMENTS
								    || mode == CommentIdentifierMode.ALL_COMMENT_LINES);
		boolean allowQuotMark = (mode == CommentIdentifierMode.QUOT_MARK_COMMENTS_ONLY
			    					 || mode == CommentIdentifierMode.ALL_COMMENT_LINES);
		
		if (mode == CommentIdentifierMode.ABAP_CODE_ONLY) {
         // only process lines that are sure to be ABAP Code
         if (AbapCult.stringStartsWith(text, ABAP.LINE_COMMENT_SIGN_STRING)) 
            return null;
         else if (AbapCult.stringStartsWith(StringUtil.trimStart(text), ABAP.COMMENT_SIGN_STRING))
            return null;
         else
         	return text;

		} else if (allowAsterisk && AbapCult.stringStartsWith(text, ABAP.LINE_COMMENT_SIGN_STRING)) {
			// text is a * comment line
			return text;

		} else if (allowQuotMark) {
         // if text is a " comment line ... 
         text = StringUtil.trimStart(text);
         if (text.length() > 1 && text.startsWith(ABAP.COMMENT_SIGN_STRING)) {
            // .. replace the comment sign " with *, otherwise the CommentIdentifier will realize that it is a " comment 
         	// (esp. with analyzeQuotMarkCommentsToo == false)
            return ABAP.LINE_COMMENT_SIGN_STRING + text.substring(1);
         } else {
            return null;
         }
	...
	public final int identifyComments(String codeText, StringBuilder output, CommentIdentifierMode mode) {
      ...
      if (output != null)
         output.append("Line number\tCode\tDecision\tAbapProbability %\tpro ABAP\tpro Text\tReasons").append(System.lineSeparator());
      ...
      for (int i = 0; i < lines.length; ++i) {
         ...
         text = filterForMode(text, mode);
         if (text == null)
         	continue;

         String nextText = (i + 1 < lines.length) ? lines[i + 1] : null;

         CommentIdentification identification = identifyComment(text, false, prevText, nextText, true);
         ++count;

         if (output != null) {
            output.append(String.valueOf(i));
            output.append('\t').append(" " + text);
            output.append('\t').append(identification.canDecide() ? (identification.isAbapCode() ? "ABAP" : "Text") : "unknown");
            output.append('\t').append(String.valueOf(identification.getAbapProbabilityPercent()));
            output.append('\t').append(String.valueOf(identification.reasonsProAbap));
	...
	public final CommentIdentification identifyComment(String lineText, boolean analyzeQuotMarkCommentsToo) {
		return identifyComment(lineText, analyzeQuotMarkCommentsToo, null, null, false);
	}
	public final CommentIdentification identifyComment(String lineText, boolean analyzeQuotMarkCommentsToo, String prevLineText, String nextLineText, boolean provideReasons) {
		...
		for (String autoGeneratedPrefix : autoGeneratedPrefixes) {
			if (AbapCult.stringStartsWith(lineText, autoGeneratedPrefix)) {
				++proAbap;
				if (reasons != null)
					reasons.append(proAbapPrefix + "line seems to be auto-generated as it starts with '" + autoGeneratedPrefix + "'");
				return new CommentIdentification(proAbap, proText, proEnglish, proGerman, -1, reasons, null, true);
			}
		}
		if (AbapCult.stringStartsWith(lineText, "***")) {
			...
			return new CommentIdentification(proAbap, proText, proEnglish, proGerman, -1, reasons, null, false);
		}

		Line line = preprocessLine(lineText, analyzeQuotMarkCommentsToo);
		int indent = line.indent;
		// if (line == null)
		// 	return new CommentIdentification(proAbap, proText, indent, reasons, line.words);

		// analyzeQuotMarkCommentsToo depends on the context:
		// - for the CommentTypeRule, a quotation mark " comment is regarded as part of ABAP code and therefore 
		//   not analyzed further (analyzeQuotMarkCommentsToo == false)
		// - for the TypoRule, " comments get a 'deep analysis' to determine whether they are textual English or German
		//   (analyzeQuotMarkCommentsToo == true)
		if (line.isQuotMarkCommentLine && !analyzeQuotMarkCommentsToo) {
			++proAbap;
			if (reasons != null)
				reasons.append(proAbapPrefix + "line starts with quotation mark \"");
			return new CommentIdentification(proAbap, proText, proEnglish, proGerman, indent, reasons, line.words, false);
		...
		Line prevLine = preprocessLine(prevLineText, analyzeQuotMarkCommentsToo);
		Line nextLine = preprocessLine(nextLineText, analyzeQuotMarkCommentsToo);
		...
		for (int i = 0; i < line.getWordCount(); ++i) {
			...
			boolean upperSameAsLower = (AbapCult.toUpper(word).equals(AbapCult.toLower(word)));

			if (!upperSameAsLower && word.length() >= 2 && Character.isUpperCase(word.charAt(0)) && word.substring(1).equals(AbapCult.toLower(word.substring(1)))) {
				++proText;
				if (reasons != null)
					reasons.append(proTextPrefix + "capitalized word '" + word + "'").append(lineSep);
			}

			if (mayBeKeyword && ABAP.isAbapUpperCaseKeyword(word)) {
				if (AbapCult.toUpper(word).equals(word)) {
					++proAbap;
					if (reasons != null)
						reasons.append(proAbapPrefix + "upper case ABAP keyword '" + word + "'").append(lineSep);
				} else {
					++proText;
					if (reasons != null)
				...
				if (nonEnglishAbapUpperCaseKeywords.contains(getHashSetKey(word))) {
					++proAbap;
					if (reasons != null)
						reasons.append(proAbapPrefix + "ABAP keyword '" + word + "'").append(lineSep);
				}

			} else if (mayBeKeyword && ABAP.isAbapLowerCaseKeyword(word)) {
				if (AbapCult.toLower(word).equals(word)) {
					++proAbap;
					if (reasons != null)
						reasons.append(proAbapPrefix + "lower case ABAP keyword '" + word + "'").append(lineSep);
				} else {
					++proText;
					if (reasons != null)
			...
			} else if (!upperSameAsLower && word.length() >= 2 && AbapCult.toUpper(word).equals(word) && !ABAP.isAbapUpperCaseKeyword(word)) {
				++proText;
				if (reasons != null)
					reasons.append(proTextPrefix + "upper case word '" + word + "' is not an ABAP keyword").append(lineSep);

			} else if (mayBeKeyword && frequentEnglishWords.contains(getHashSetKey(word))) {
				++proText;
				++proEnglish;
				if (reasons != null)
					reasons.append(proTextPrefix + "non-ABAP English word '" + word + "'").append(lineSep);

			} else if (mayBeKeyword && frequentGermanWords.contains(getHashSetKey(word))) {
				++proText;
				++proGerman;
				if (reasons != null)
					reasons.append(proTextPrefix + "non-ABAP German word '" + word + "'").append(lineSep);

			} else if (mayBeKeyword && frequentEnglishTypos.containsKey(word)) {
				++proText;
				++proEnglish;
				if (reasons != null)
					reasons.append(proTextPrefix + "non-ABAP English typo '" + word + "'").append(lineSep);

			} else if (mayBeKeyword && frequentBritishEnglish.containsKey(word)) {
				++proText;
				++proEnglish;
				if (reasons != null)
					reasons.append(proTextPrefix + "non-ABAP British English '" + word + "'").append(lineSep);

			} else if (mayBeKeyword && StringUtil.indexOfAny(word, GERMAN_SPECIAL_CHARS, 0) >= 0) {
				++proText;
				++proGerman;
				if (reasons != null)
					reasons.append(proTextPrefix + "non-ABAP word with German special char '" + word + "'").append(lineSep);

			} else {
				if (word.indexOf('_') >= 0) {
					++proAbap;
					if (reasons != null)
						reasons.append(proAbapPrefix + "underscore(s) in '" + word + "'").append(lineSep);
				}
		...
		String lastSep = null;
		for (int sepIndex = 0; sepIndex < line.getSeparatorCount(); ++sepIndex) {
			String sep = line.separators[sepIndex];
			if (lastSep != null && AbapCult.stringEndsWith(lastSep, "<") && AbapCult.stringStartsWith(sep, ">")) {
				++proAbap;
				if (reasons != null)
					reasons.append(proAbapPrefix + "field-symbol identifier <...>");
			}

			String key = getHashSetKey(sep);
			if (abapSeparators.contains(key)) {
				++proAbap;
				if (reasons != null)
					reasons.append(proAbapPrefix + "typical ABAP separator '" + sep + "'").append(lineSep);
			} else if (textSeparators.contains(key)) {
				++proText;
				if (reasons != null)
					reasons.append(proTextPrefix + "typical text separator '" + sep + "'").append(lineSep);
			}

			if (sepIndex + 1 < line.separators.length && sep.startsWith(ABAP.DOT_SIGN_STRING)) {
				++proText;
				if (reasons != null)
					reasons.append(proTextPrefix + "period . inside line").append(lineSep);
			}

			if (sepIndex >= 2 && sep.startsWith(ABAP.COLON_SIGN_STRING)) {
				++proText;
				if (reasons != null)
		...
		if (prevLine != null) {
			int indentChange = prevLine.expectedIndentChangeForNextLine();
			if (indentChange != 0 && prevLine.indent + indentChange == line.indent) {
				++proAbap;
				if (reasons != null)
					reasons.append(proAbapPrefix + getIndentChangeInfo(indentChange) + " after " + prevLine.words[0]);
			}
			indentChange = line.expectedIndentChangeForThisLine();
			if (indentChange != 0 && prevLine.indent + indentChange == line.indent) {
				++proAbap;
				if (reasons != null)
					reasons.append(proAbapPrefix + getIndentChangeInfo(indentChange) + " with " + line.words[0]);
			}
		}

		if (nextLine != null) {
			int indentChange = line.expectedIndentChangeForNextLine();
			if (indentChange != 0 && line.indent + indentChange == nextLine.indent) {
				++proAbap;
				if (reasons != null)
					reasons.append(proAbapPrefix + getIndentChangeInfo(indentChange) + " after " + line.words[0]);
			}
			indentChange = nextLine.expectedIndentChangeForThisLine();
			if (indentChange != 0 && line.indent + indentChange == nextLine.indent) {
				++proAbap;
				if (reasons != null)
					reasons.append(proAbapPrefix + getIndentChangeInfo(indentChange) + " with " + nextLine.words[0]);
			}
		}

		// if nothing was detected, consider some special cases
		if (proAbap == 0 && proText == 0) {
			String comment = line.comment;
			if (textStartsWithActualQuote(comment)) {
				++proText;
				if (reasons != null)
			...
			if (!StringUtil.isNullOrEmpty(prevLineText)) {
				CommentIdentification prevResult = identifyComment(prevLineText, analyzeQuotMarkCommentsToo);
				if (prevResult.canDecide()) {
					if (reasons != null)
						reasons.append("decision based on previous line");
					return new CommentIdentification(prevResult.reasonsProAbap, prevResult.reasonsProText, prevResult.reasonsProEnglish, prevResult.reasonsProGerman, indent, reasons, line.words, false);
				}
			}
			if (!StringUtil.isNullOrEmpty(nextLineText)) {
				CommentIdentification nextResult = identifyComment(nextLineText, analyzeQuotMarkCommentsToo);
				if (nextResult.canDecide()) {
					if (reasons != null)
						reasons.append("decision based on next line");
					return new CommentIdentification(nextResult.reasonsProAbap, nextResult.reasonsProText, nextResult.reasonsProEnglish, nextResult.reasonsProGerman, indent, reasons, line.words, false);
				}
			}
	...
	private static Line preprocessLine(String text, boolean analyzeQuotMarkCommentsToo) {
		final String separatorDelimiters = ABAP.COMMENT_SIGN_STRING + ABAP.QUOT_MARK_STRING + ABAP.QUOT_MARK2_STRING + ABAP.PIPE_STRING + ABAP.BRACE_CLOSE_STRING;
		final String allowedCharsInWord = "_-'";
		final String allowedCharsAfterKeyword = " .:(";
		...
		// see .identifyComment() for an explanation of analyzeQuotMarkCommentsToo
		if (text.startsWith(ABAP.COMMENT_SIGN_STRING) && !analyzeQuotMarkCommentsToo) {
		...
		// identify words, excluding literals in '...', |...|, |...{, }...|, as well as line-end comments "
		...
		while (readPos < text.length()) {
			...
			} else if (text.charAt(readPos) == ABAP.COMMENT_SIGN) {
			...
			if (curChar == ABAP.COMMENT_SIGN) {
				...
				if (readPos > wordStart) {
					...
					if (wordStart > 0 && readPos < text.length() && text.charAt(wordStart - 1) == '<' && text.charAt(readPos) == '>' && Character.isLetter(text.charAt(wordStart)))
						newWord = "<" + newWord + ">";
					words.add(newWord);
	...
	private static int skipUntil(String text, int readPos, char[] delimiterChars) {
		int wordEnd = StringUtil.indexOfAny(text, delimiterChars, readPos + 1); // skip the char at readPos
	...
	public static String getScopeDescription(CommentIdentifierMode mode) {
		switch (mode) {
			case ABAP_CODE_ONLY:
				return " for ABAP code lines";
			case QUOT_MARK_COMMENTS_ONLY:
				return " for \" comment lines";
			case ASTERISK_COMMENTS:
				return " for * comment lines";
			case ALL_COMMENT_LINES:
				return " for \" and * comment lines";
			default:
	...
	public void addCommentSamples(String[] lines, CommentIdentifierMode mode) {
      for (String line : lines) {
         if (StringUtil.isNullOrEmpty(line))
            continue;

         // skip lines that do NOT start with " or *
         String text = filterForMode(line, mode);
         if (text == null)
         	continue;

         // skip lines that are currently rated as ABAP Code
         CommentIdentification identification = identifyComment(text, true, null, null, false);
         if (identification.isAbapCode())
         	continue;

         // skip certain lines completely
         boolean skipLine = false;
         for (String skipText : wordFreqSkipTexts) {
         ...
         for (String removeText : wordFreqRemoveTexts) {
	...
	private boolean isTextualWord(String word) {
   	...
   	if (word.startsWith("sy-") || word.startsWith("SY-") || word.startsWith("syst-") || word.startsWith("SYST-"))
   		return false;
