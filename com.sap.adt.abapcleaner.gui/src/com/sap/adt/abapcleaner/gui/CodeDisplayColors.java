package com.sap.adt.abapcleaner.gui;

import org.eclipse.swt.graphics.Color;

/***
 * Encapsulates the Colors used by the {@link CodeDisplay}. 
 * Depending on the method background color supplied to the constructor, a bright or a dark theme is selected.
 */
public class CodeDisplayColors {
	private static final boolean USE_DARK_BY_DEFAULT = false;
	
	public final Color background;

	// text fore colors
	public final Color textIdentifier;
	public final Color textIdentifierWritePos;
	public final Color textComment;
	public final Color textKeyword;
	public final Color textDeclarationKeyword;
	public final Color textNumber;
	public final Color textOperator;
	public final Color textStringLiteral;
	public final Color textTokenOperator;
	public final Color textNonAbap;

	// line numbers
	public final Color lineNumberBack;
	public final Color lineNumber;

	// background colors for lines 
	public final Color lineSkip;
	public final Color lineDeleted;
	public final Color lineAdded;
	public final Color lineChanged;
	public final Color lineHighlight;
	public final Color lineOutsideCleanupRange;
	
	// background colors for selected lines 
	public final Color selLine;
	public final Color selLineSkip;
	public final Color selLineDeleted;
	public final Color selLineAdded;
	public final Color selLineChanged;
	public final Color selLineHighlight;
	public final Color selBorder;

	// search
	public final Color highlightSearchMatch;
	public final Color normalSearchMatch;

	// vertical line
	public final Color verticalLine;

	public static CodeDisplayColors createDefault() {
		return new CodeDisplayColors(null, null, null, null, null, null); 
	}
	
	/**
	 * Creates a set of Colors used in the {@link CodeDisplay}; all parameters may be null. 
	 * 
	 * @param methodBackground - if supplied, it is used to determine whether a bright or dark theme is used
	 * @param identifier - text color of identifiers
	 * @param keyword - text color of ABAP keywords
	 * @param literal - text color of string literals
	 * @param literalNumber - text color of integer literals
	 * @param comment - text color of comments 
	 */
	public CodeDisplayColors(Color methodBackground, Color identifier, Color keyword, Color literal, Color literalNumber, Color comment) {
		// if a method background color was supplied, derive from it whether to use a bright or a dark theme 
		boolean useDark = getUseDarkTheme(methodBackground);
		
		//                                 | dark theme  |        | bright theme | existing ADT preference
		// ----------------------------------------------+-----------------------+------------------------
		background = 				 createColor( 40,  40,  40, useDark, 255, 255, 255, methodBackground);
		
		// text colors
		textIdentifier = 	 		 createColor(255, 255, 255, useDark,   0,   0,   0, identifier);
		textIdentifierWritePos = createColor(216,  12,  12, useDark, 216,  12,  12, null); 
		textOperator = 		 	 createColor(104, 151, 187, useDark, 128,   0, 128, null); // ADT reuses the 'identifier' color here  
		textTokenOperator = 		 createColor(104, 151, 187, useDark, 159,  52,   0, null); // ADT reuses the 'identifier' color here
		textKeyword = 				 createColor( 86, 165, 228, useDark,   0,   0, 255, keyword);
		textDeclarationKeyword = createColor(  0, 185, 185, useDark,   0, 153, 153, null);
		textStringLiteral = 		 createColor( 30, 181,  64, useDark,  77, 166,  25, literal);
		textNumber = 				 createColor( 91, 233, 254, useDark,  51, 153, 255, literalNumber);
		textComment = 				 createColor(128, 128, 128, useDark, 128, 128, 128, comment);
		textNonAbap = 	   		 createColor(255, 255, 255, useDark,  64,  64,  64, null); 

		// line numbers
		lineNumberBack = 			 createColor( 72,  72,  72, useDark, 220, 220, 220, null);
		lineNumber = 				 createColor(208, 208, 208, useDark,  48,  48,  48, null);

		// line background colors
		// (dark theme colors inspired by code coverage colors in Eclipse; light-theme colors inspired by Compare plug-in of Notepad++)
		// cp. FrmProfiles.open() for highlighting new rules or configuration options (using color for "added") / enhanced rules (using color for "changed") 
		lineSkip = 					 createColor( 20,  20,  20, useDark, 225, 225, 225, null);
		lineDeleted = 				 createColor( 98,   5,   5, useDark, 255, 215, 215, null); // Eclipse (light): 255, 160, 160
		lineAdded = 				 createColor(  6,  74,   6, useDark, 217, 255, 217, null); // Eclipse (light): 192, 255, 192 
		lineChanged = 				 createColor( 98,  98,   0, useDark, 231, 231, 152, null); // Eclipse (light): 255, 255, 128 
		lineHighlight = 			 createColor(160,  74,   6, useDark, 240, 191,  94, null); // Eclipse (light)-like: 255, 192, 0  
		lineOutsideCleanupRange =  createColor(98, 98,  98, useDark, 200, 200, 200, null);

		// background colors for lines within the current selection of lines
		selLine = 				 	 createColor(  0,   0,   0, useDark, 246, 246, 255, null);
		selLineSkip = 		 		 createColor(  8,   8,   8, useDark, 227, 227, 236, null); 
		selLineDeleted = 	 		 createColor( 64,   4,   4, useDark, 246, 221, 230, null); 
		selLineAdded = 			 createColor(  4,  64,   4, useDark, 222, 246, 231, null);
		selLineChanged = 			 createColor( 64,  64,   0, useDark, 231, 231, 191, null);
		selLineHighlight = 		 createColor(106,  50,   4, useDark, 237, 206, 155, null);
		selBorder = 				 createColor(192, 110,   0, useDark, 255, 165,   0, null);

		// search matches
		highlightSearchMatch = 	 createColor(243, 236, 121, useDark, 139,   0,   0, null);
		normalSearchMatch = 		 createColor(192, 192, 192, useDark,   0,   0,   0, null);

		// vertical line
		verticalLine = 			 createColor(255, 182, 193, useDark, 255, 182, 193, null); 
	}
	
	static boolean getUseDarkTheme(Color background) {
		return (background == null) ? USE_DARK_BY_DEFAULT : (background.getRed() + background.getGreen() + background.getBlue() < 3 * 128);
	}

	static Color createColor(int darkR, int darkG, int darkB, boolean useDarkTheme, int brightR, int brightG, int brightB, Color existingColor) {
		if (existingColor != null) {
			return existingColor;
		
		} else if (useDarkTheme) {
			return new Color(darkR, darkG, darkB);

		} else { // bright theme
			return new Color(brightR, brightG, brightB);
		}
	}
}
